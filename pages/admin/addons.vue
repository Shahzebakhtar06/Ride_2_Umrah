  ...
  <div>
    <div id="add-edit-amenities" class="admin-layout">
      <div class="page-header">
        <div class="title">Manage Your Addons</div>
        <div class="add-btn">
      ...
      <div class="page-content">
        ...
        >
          <a-form-model
            ...
            :wrapperCol="{ span: 24 }"
          >
            <a-form-model-item has-feedback label="Addon Name" prop="name">
              <a-input
                v-model="form.name"
                type="text"
                autocomplete="off"
            ...
            <a-form-model-item has-feedback label="Addon Price" prop="price">
              <a-input
                v-model="form.price"
                type="text"
                autocomplete="off"
...
export default {
  ...
  computed: {
    modalTitle() {
      return this.renderingFor == "Add" ? "Add New Addon" : "Edit Your Addon";
    },
  },
  data() {
    return {
      ...
      rules: {
        ...
        type: [
          {
            required: true,
            message: "Addon Type is required!",
            trigger: "blur",
          },
        ],
        image: [
  ...
  methods: {
    getImageUrl(imagePath) {
      let url =
        "https://expedia-api.savvyskymart.com/uploads/addons/" + imagePath;
      return url;
    },
    onFileChange(event) {
    ...
    queryData(params) {
      return this.$axios.get("addon", { params: params });
    },
    showModal() {
      this.visible = true;
    },
    ...
    async handleItemDelete(val) {
      ...
      if (val.id) {
        this.$confirm({
          title: "Are you sure delete this Addon?",
          okText: "Yes",
          okType: "danger",
          okButtonProps: {
            loading: isDeleting,
          ...
          onOk: async () => {
            ...
            try {
              let res = await this.$axios.delete(`addon/${val.id}`);
              if (res.status == 200) {
                this.$notification.success({
